...
simple_matrix = np.array(simple_matrix)
goal = (simple_matrix.shape[0]-1, simple_matrix.shape[1]-1) # Bottom-right corner
ship_path, ship_path_length = a_star_v1.a_star_search_8(simple_matrix, start, goal,5)
ship_path = ship_path[::-1]  # Reverse path to start from the beginning
...
#3: when only moving in 4 directions
...
#4: when able to move diagonal 
shortest_path, shortest_path_length = dijkstra_v4.dijkstra_path_8(matrix,5)
ship_path = shortest_path
print("shortest_path_length for dijkstra algo  8-direcion is: ", shortest_path_length)
