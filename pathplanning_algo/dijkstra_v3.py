# version 3:
# add restrictions to the weight of each path, while still using shortest average path finding method

from math import inf

def find_all(weighted_matrix,limit, start, end=-1, ):
    """
    ...
    for count in range(n-1):
        ...
        for v in range(n):
            if not(spVertex[v]) and weighted_matrix[u][v] != 0 and dist[u] + weighted_matrix[u][v] < dist[v] and weighted_matrix[u][v] < limit:
                parent[v] = u
...
def find_shortest_path(wmat, limit, start, end=-1):
    """
    Returns paths' list of all remaining vertices.
    """
    return find_all(wmat, limit, start, end)[1]



def find_shortest_distance(wmat, limit, start, end=-1):
    """
    Returns distances' list of all remaining vertices.
    """
    return find_all(wmat, limit, start, end)[0]
