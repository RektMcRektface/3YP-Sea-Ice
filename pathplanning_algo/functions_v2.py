...
def averagingandspeed(matrix, path,speed_list):
    overall_score = 0
    overall_time = 0
    a =0
    b=0 
    ...
    for i,j in path:
        ...
        if a-i != 0 and b-j !=0:
            overall_time = overall_time + math.sqrt(2)/speed_list[matrix[i][j]]
        else:
            overall_time = overall_time + 1/speed_list[matrix[i][j]]
        a = i
        b = j
