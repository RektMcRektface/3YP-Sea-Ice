...
xc_windowed = xc_combined .* window; % Apply the window to the LFM signal

% Calculate the matched filter output for the combined signal
[pc_c, lags] = xcorr(xc_windowed, xc_base); % Using the base LFM signal as the template
...
title('Matched Filter Output for Two Targets with Windowing');
